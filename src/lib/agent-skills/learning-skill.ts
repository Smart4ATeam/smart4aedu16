...
### 8. 我的時間軸（整合：開課梯次 + 全站公告 + 個人提醒）⭐ 預設首選
\`GET /api-agent-my-agenda\`

**這是處理「X 月有什麼？」「這週有什麼安排？」這類模糊時間問句的首選 endpoint。**
一次回傳三類事件並依日期排序，Agent 不必再合併。

可選查詢參數：
- \`date_from\` / \`date_to\`：YYYY-MM-DD，**預設 = 今天 ~ 今天+90 天**
- \`types\`：逗號分隔，可選 \`course_session,calendar_global,calendar_personal\`，預設三者全部
- \`course_id\`：限定某門課的梯次（只影響 course_session 類）
- \`category\`：限定某類課程，例如 \`basic\`（只影響 course_session 類）
...
\`\`\`json
{
  "agenda": [
    {
      "type": "course_session",
      "id": "...", "date": "2026-05-10", "time": null,
      "title": "AI 基礎課（5月梯）",
      "description": "台北 ｜ ~2026-05-11",
      "location": "台北",
      "registration_url": "https://...",
      "course": { "id": "...", "title": "...", "course_code": "...", "category": "basic", "price": 6000 },
      "extra": { "status": "open", "schedule_type": "single", "end_date": "...", "max_students": 20, "session_price": 6000 }
    },
    {
      "type": "calendar_global",
      "id": "...", "date": "2026-05-15", "time": "19:30:00",
      "title": "AI 工具線上講座", "description": "...", "color": "gradient-blue"
    },
    {
      "type": "calendar_personal",
      "id": "...", "date": "2026-05-20", "time": null,
      "title": "提醒：交期末報告", "description": "", "color": "gradient-orange"
    }
  ],
  "counts": { "course_session": 4, "calendar_global": 2, "calendar_personal": 1, "total": 7 },
  "range": { "date_from": "2026-04-21", "date_to": "2026-07-20" }
}
\`\`\`

> 三類事件本質：
> - \`course_session\`：可報名的開課梯次（含 \`registration_url\`）
> - \`calendar_global\`：管理員公告的全站活動（線上講座、社群聚會等）
> - \`calendar_personal\`：學員自己建立的提醒（可用 \`/api-agent-my-calendar\` 異動）

### 9. 我的行事曆（細查 / 寫入個人事件）
\`GET /api-agent-my-calendar\`

> 一般查詢請優先用上面的 \`/api-agent-my-agenda\`。本端點用於只想看 \`calendar_events\`、不要梯次混入時。
...
- 「我下週有什麼活動？」「5 月有什麼？」「下個月有哪些事」（**模糊時間問句**）
  → \`GET /api-agent-my-agenda?date_from=YYYY-MM-DD&date_to=YYYY-MM-DD\`（首選）

- 「下週有哪些**講座/公告活動**？」
  → \`GET /api-agent-my-agenda?types=calendar_global&date_from=...&date_to=...\`

- 「我自己排了什麼提醒？」
  → \`GET /api-agent-my-agenda?types=calendar_personal&date_from=...&date_to=...\`
...
- 問「X 月有什麼？」「這週有什麼安排？」「下個月有哪些事」這類**模糊時間問句** → **必用** \`/api-agent-my-agenda\`，不要只查梯次或行事曆其中一邊。
- 問「X 月有哪些**課**/開課/梯次」（明確要看課程） → \`/api-agent-my-agenda?types=course_session&...\` 或 \`/api-agent-sessions\`。
