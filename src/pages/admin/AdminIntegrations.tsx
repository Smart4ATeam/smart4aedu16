...
const endpoints: ApiEndpoint[] = [
  ...
  },
  {
    ...
    path: "/api-reg-order",
    authType: "x-api-key (API_INTEGRATION_KEY)",
    description: "供 Make.com 呼叫，只寫入 reg_orders（payment_status=pending）。不會建立學員或報名明細，待付款後再呼叫 api-reg-split 拆解。支援 1~3 人 × 1~4 課程。",
    requiredFields: [
  ...
  },
  {
    ...
    path: "/api-reg-split",
    authType: "x-api-key (API_INTEGRATION_KEY)",
    description: "付款確認後呼叫，根據 order_no 自動拆解：從訂單的 p1~p3 比對/建立 reg_members，再為每人 × 每課程建立 reg_enrollments。需訂單 payment_status=paid 才可拆解，且不可重複拆解。",
    requiredFields: [
