...
const endpoints: ApiEndpoint[] = [
  ...
  {
    id: "api-reg-enrollment",
    name: "課程報名（reg_ 系統）",
    icon: <Users className="w-4 h-4" />,
    method: "POST",
    path: "/api-reg-enrollment",
    authType: "x-api-key (API_INTEGRATION_KEY)",
    description: "供 Make.com 呼叫，接收報名訂單資料後自動建立/比對學員（reg_members）、建立訂單（reg_orders）、並拆解為每人×每課程的報名明細（reg_enrollments）。支援 1~3 人 × 1~4 課程。",
    requiredFields: [
      { name: "order_no", type: "string", required: true, desc: "訂單編號（唯一）" },
      { name: "course_codes", type: "string[]", required: true, desc: "課程代碼陣列（對應 reg_courses.course_code，最多 4 個）" },
      { name: "persons", type: "object[]", required: true, desc: "報名人員陣列（1~3 人），每人需有 name，可選 phone、email" },
    ],
    optionalFields: [
      { name: "payment_status", type: "string", desc: "付款狀態：pending / paid（預設 pending）" },
      { name: "paid_at", type: "string", desc: "付款時間（ISO 8601）" },
      { name: "payment_method", type: "string", desc: "付款方式" },
      { name: "total_amount", type: "number", desc: "總金額" },
      { name: "discount_plan", type: "string", desc: "折扣方案：single / duo / trio" },
